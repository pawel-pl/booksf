...
public class SubArrayOAnd1 {
	...
	private static int findSubArray(int arr[], int n) {
		...
		// values.
		// If you see a value again, that it is a case of sumleft[i] =
		// sumleft[j]. Check
		// if this j-i is more than maxsize.
		// The optimum size of hash will be max-min+1 as these many different
		// values
		// of sumleft[i] are possible. Since we use optimum size, we need to
		// shift
		...
		if (maxsize == -1)
			...
			System.out.println(String.format("%d to %d", startindex, startindex + maxsize - 1));
