...
public class ClosestPair {
	...
	private static Pair divideAndConquer(List<? extends Point> pointsSortedByX, List<? extends Point> pointsSortedByY) {
		...
		List<? extends Point> leftPlane = pointsSortedByX.subList(0, dividingIndex);
		List<? extends Point> rightPlane = pointsSortedByX.subList(dividingIndex, numPoints);

		List<Point> tempList = new ArrayList<Point>(leftPlane);
		sortByY(tempList);
		Pair closestPair = divideAndConquer(leftPlane, tempList);

		tempList.clear();
		tempList.addAll(rightPlane);
		sortByY(tempList);
		Pair closestPairRight = divideAndConquer(rightPlane, tempList);
		...
		double centerX = rightPlane.get(0).x;
		for (Point point : pointsSortedByY)
			if (Math.abs(centerX - point.x) <= shortestDistance / 2)
				tempList.add(point);
