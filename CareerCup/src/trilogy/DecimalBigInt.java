...
/**
 * A big-number class for natural numbers (i.e. nonnegative), which uses a
 * decimal-based format (instead of a binary based one like the
 * {@link java.math.BigInteger}).
 * <p>
 * The numbers are stored in a positional notation with radix 1000000000 (10^9),
 * which supports easier conversion from/to decimal.
 * </p>
 * <p>
 * Until now we only have addition and multiplication, no subtraction or
 * division.
 * </p>
 * <p>
 * See my answer <a href=
 * "http://stackoverflow.com/questions/5318068/very-large-numbers-in-java-without-using-java-math-biginteger/5318896#5318896"
 * >Creating a simple Big number class in Java</a> on Stackoverflow for details
 * on how it is done.
 * </p>
 * 
...
public class DecimalBigInt implements Comparable<DecimalBigInt> {
	...
	/**
	 * The number of decimal digits fitting in one digit of our system.
	 */
	private final static int RADIX_DECIMAL_DIGITS = 9;

	/**
	 * big-endian representation of the digits. Little-endian would have been
	 * smarter, but now I don't want to change it everywhere.
	 */
	private int[] digits;
	 ...
	 * 
	 * @param digits
	 *            a list of digits, each between 0 (inclusive) and {@link RADIX}
	 *            (exclusive).
	...
	/**
	 * A simple string view for debugging purposes. (Will be replaced later with
	 * a real decimal conversion.)
	 */
	 ...
	 *             if the number is not in correct decimal format, e.g. if it
	 *             contains any characters outside of 0..9.
	 */
	public static DecimalBigInt valueOf(String decimal) {
		...
		for (int i = 0; i < bigLen; i++) {
			String block = decimal.substring(Math.max(firstSome + (i - 1) * RADIX_DECIMAL_DIGITS, 0), firstSome + i
					* RADIX_DECIMAL_DIGITS);
	...
	/**
	 * creates a DecimalBigInt from a string-representation in some arbitrary
	 * radix.
	 * 
	 * Note: this is not the most efficient implementation, since we use the
	 * Horner scheme on DecimalBigInt values, instead working directly on
	 * {@code int[]} and convert to a DecimalBigInt only in the end.
	 * 
	 * @param text
	 *            the big-endian string representation of the number, using the
	 *            decimal digits '0' ... '9' and additionally the latin letters
	 *            'A' ... 'Z' (or 'a' ... 'z'). (Only letters below the given
	 *            radix are allowed, of course.)
	 * @param radix
	 *            the radix used in the representation, between
	...
	public static DecimalBigInt valueOf(String text, int radix) {
		...
		for (char digit : text.toCharArray()) {
			...
			if (iDigit < 0) {
				throw new NumberFormatException("digit " + digit + " is not a valid base-" + radix + "-digit.");
			}
	...
	/**
	 * creates a DecimalBigInt from a representation in some arbitrary radix.
	 * 
	 * @param digits
	 *            the individual digits, each between 0 (inclusive) and radix,
	 *            exclusive.
	 * @param radix
	 *            the radix of the representation, an arbitrary value >= 2.
	...
	public String toDecimalString() {
		StringBuilder b = new StringBuilder(RADIX_DECIMAL_DIGITS * digits.length);
	...
	/**
	 * Converts the number to a String in a given radix. This uses
	 * {@link Character.digit} to convert each digit to one character.
	 * 
	 * @param radix
	 *            the radix to use, between {@link Character.MIN_RADIX} and
	 *            {@link Character.MAX_RADIX}.
	 * @return a String containing the digits of this number in the specified
	 *         radix, using '0' .. '9' and 'a' .. 'z' (as much as needed).
	 */
	...
	/**
	 * adds one digit from the addend to the corresponding digit of the result.
	 * If there is carry, it is recursively added to the next digit of the
	 * result.
	 */
	...
	/**
	 * multiplies two digits and adds the product to the result array at the
	...
	private void multiplyDigits(int[] result, int resultIndex, int[] leftFactor, int[] rightFactor) {
		for (int i = 0; i < leftFactor.length; i++) {
			for (int j = 0; j < rightFactor.length; j++) {
				...
				multiplyDigit(result, resultIndex - (i + j), leftFactor[leftFactor.length - i - 1],
	...
	/**
	 * does one step in the short division algorithm, i.e. divides a two-digit
	 * number by a one-digit one.
	 * 
	 * @param result
	 *            the array to put the quotient digit in.
	 * @param resultIndex
	 *            the index in the result array where the quotient digit should
	 *            be put.
	 * @param divident
	 *            the last digit of the divident.
	 * @param lastRemainder
	 *            the first digit of the divident (being the remainder of the
	 *            operation one digit to the left). This must be < divisor.
	 * @param divisor
	 *            the divisor.
	...
	/**
	 * The short division algorithm, like described in <a
	 * href="http://en.wikipedia.org/wiki/Short_division">Wikipedia's article
	 * <em>Short division</em></a>.
	 * 
	 * @param result
	 *            an array where we should put the quotient digits in.
	 * @param resultIndex
	 *            the index in the array where the highest order digit should be
	 *            put, the next digits will follow.
	 * @param divident
	 *            the array with the divident's digits. (These will only be
	 *            read, not written to.)
	 * @param dividentIndex
	 *            the index in the divident array where we should start
	 *            dividing. We will continue until the end of the array.
	 * @param divisor
	 *            the divisor. This must be a number smaller than {@link #RADIX}
	 *            .
	 ...
	 * 
	 * @param divisor
	 *            an integer with {@code 0 < divisor < RADIX}.
	 * @return the integer part of the quotient {@code this / divisor}, ignoring
	 *         the remainder.
	 * @throws IllegalArgumentException
	 *             if the divisor is <= 0 or >= RADIX.
	 */
	 ...
	 * 
	 * @param radix
	 *            the target radix, {@code 1 < radix < RADIX}.
	 * @return the digits of this number in the base-radix system, in big-endian
	 *         order.
	 */
	...
	/**
	 * compares this object with another object for equality. A DecimalBigInt is
	 * equal to another object only if this other object is also a DecimalBigInt
	 * and both represent the same natural number.
	 */
	...
	/**
	 * calculates the factorial of an int number. This uses a simple iterative
	 * loop.
	 * 
	 * Each 1000 factors we print the current digit count (in our internal
	 * radix) to the standard error stream.
	 * 
	 * @param n
	 *            should be < RADIX (but you supposedly don't want to use bigger
	 *            number anyway, as it takes years).
	 */
	public static DecimalBigInt factorial(int n) {
		...
		for (int i = 2; i <= n; i++) {
			...
			if (i % 1000 == 0) {
				System.err.println("log_RADIX(fac(" + i + ")) = " + fac.digits.length);
			}
		}
		return fac;
	}
